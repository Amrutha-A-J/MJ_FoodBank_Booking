...
- Push notifications use Firebase Cloud Messaging; clients and volunteers register tokens via `/notifications/register`, and the server sends alerts for booking changes and upcoming shifts. Configure `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, and `FIREBASE_PRIVATE_KEY`.
