...
- `PantrySchedule` shows daily pantry availability with holidays, blocked slots, and breaks rendered as non bookable times. Each time block supports up to four shoppers across Slot 1–Slot 4, displaying how many bookings exist per slot. Staff can book appointments for walk-ins.
...
### Clients and Staff
- **Clients** book pantry appointments through a calendar and bookings are automatically approved or rejected based on monthly usage.
- **Staff** can cancel or reschedule bookings and assign clients directly to slots. Rejections and cancellations require a reason.
- Clients can view a booking history table listing all approved appointments with Cancel and Reschedule options.
...
- **ManageAvailability** – lets staff maintain holidays, blocked slots, and recurring breaks.
 - **PantrySchedule** – primary staff tool to view bookings per time block and book walk-in appointments, while marking holidays, blocked slots, and breaks as non bookable entries in the schedule.
...
- **bookings** – PK `id`; FK `user_id` → `users.id`; FK `slot_id` → `slots.id`; `status` in `approved|rejected|preapproved|cancelled`; includes `reschedule_token`.
...
### Bookings
- `POST /bookings` → `{ message: 'Booking automatically approved'|'Booking automatically rejected', bookingsThisMonth, status, rescheduleToken }`
...
- `POST /bookings/reschedule/:token` → `{ message: 'Booking rescheduled', status, rescheduleToken }`
- `POST /bookings/preapproved` → `{ message: 'Preapproved booking created', rescheduleToken }`
- `POST /bookings/staff` → `{ message: 'Booking created for client', status, rescheduleToken }`
