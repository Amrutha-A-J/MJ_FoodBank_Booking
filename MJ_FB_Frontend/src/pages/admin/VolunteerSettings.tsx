...
export default function VolunteerSettings() {
  ...
  function openShiftDialog(
    categoryId: number,
    init: { roleName: string } & Partial<Omit<typeof shiftDialog, 'categoryId' | 'open' | 'roleName'>>,
  ) {
    setShiftDialog({
      open: true,
      slotId: init.slotId,
  ...
  async function saveSubRole() {
    try {
      if (!subRoleDialog.roleName || !subRoleDialog.startTime || !subRoleDialog.endTime) {
      ...
      setSubRoleDialog({ open: false, roleName: '', startTime: '', endTime: '', maxVolunteers: '1', isWednesdaySlot: false });
      loadData();
    } catch (e) {
  ...
  async function saveShift() {
    try {
      if (!shiftDialog.startTime || !shiftDialog.endTime) {
        handleSnack('All fields are required', 'error');
        return;
      }
                      ...
                      <Grid container alignItems="center" spacing={1}>
                        ...
                        <Grid item>
                          <Button
                            ...
                            variant="outlined"
                            startIcon={<AddIcon />}
                            onClick={() => openShiftDialog(master.id, { roleName: role.name, maxVolunteers: role.max_volunteers.toString() })}
                          >
                            Add Shift
                          </Button>
                        </Grid>
      ...
      <Dialog open={subRoleDialog.open} onClose={() => setSubRoleDialog({ ...subRoleDialog, open: false })} fullWidth>
        ...
        <DialogContent>
          <TextField
            ...
            onChange={e => setSubRoleDialog({ ...subRoleDialog, roleName: e.target.value })}
          />
          <TextField
            margin="dense"
            label="Start Time"
