...
describe('Aggregations page', () => {
  ...
  it('submits manual aggregate', async () => {
    render(<Aggregations />);
    await waitFor(() => expect(mockGetWarehouseOverall).toHaveBeenCalled());
    fireEvent.click(screen.getByRole('tab', { name: /yearly overall aggregations/i }));
    ...
    fireEvent.change(screen.getByLabelText(/donations/i), { target: { value: '10' } });
    fireEvent.change(screen.getByLabelText(/surplus/i), { target: { value: '5' } });
    ...
    await waitFor(() => expect(mockPostManualWarehouseOverall).toHaveBeenCalled());
