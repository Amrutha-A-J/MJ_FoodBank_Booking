...
- A nightly volunteer no-show cleanup job runs via `node-cron` at `0 20 * * *` Regina time to mark past approved volunteer bookings as `no_show` after `VOLUNTEER_NO_SHOW_HOURS` (default `24`) hours and emails coordinators about the changes using the `VOLUNTEER_NO_SHOW_NOTIFICATION_TEMPLATE_ID` template.
...
The volunteer no-show cleanup job waits `VOLUNTEER_NO_SHOW_HOURS` (default `24`) hours after a shift before marking it as `no_show` and notifies coordinators using `VOLUNTEER_NO_SHOW_NOTIFICATION_TEMPLATE_ID`.
