...
- Tests run with a unified setup file (`tests/setupTests.ts`) that imports `tests/setupEnv.ts` and `tests/setupFetch.ts`; the latter polyfills `global.fetch` with `undici`.
