...
export async function setupDatabase() {
  ...
  if (tableCheck.rows[0].slots_exists && tableCheck.rows[0].clients_exists) {
    ...
    const staffAccessRes = await client.query(`
      SELECT column_name FROM information_schema.columns
    ...
    const profileLinkRes = await client.query(`
      SELECT column_name FROM information_schema.columns
    ...
    const eventCreatedByRes = await client.query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'events' AND column_name = 'created_by';
    `);
    if ((eventCreatedByRes.rowCount ?? 0) === 0) {
      await client.query(
        `ALTER TABLE events ADD COLUMN created_by integer REFERENCES public.staff(id) ON DELETE CASCADE;`
      );
      await client.query(
    ...
    const anonymousVisitRes = await client.query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'client_visits' AND column_name = 'is_anonymous';
    `);
    if ((anonymousVisitRes.rowCount ?? 0) === 0) {
      await client.query(
        `ALTER TABLE client_visits ADD COLUMN is_anonymous boolean NOT NULL DEFAULT false;`
      );
    }
    ...
    `);
    if (!recurringTableRes.rows[0].exists) {
      await client.query(`
        CREATE TABLE volunteer_recurring_bookings (
    ...
    const recurringIdRes = await client.query(`
      SELECT column_name FROM information_schema.columns
    ...
    const vbReasonRes = await client.query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'volunteer_bookings' AND column_name = 'reason';
    `);
    if ((vbReasonRes.rowCount ?? 0) === 0) {
      await client.query(
        `ALTER TABLE volunteer_bookings ADD COLUMN reason text;`
      );
    }
    await client.query(`
    ...
    `);
    await client.query(`
      ALTER TABLE volunteer_roles DROP CONSTRAINT IF EXISTS volunteer_roles_category_id_fkey;
      ALTER TABLE volunteer_roles ADD CONSTRAINT volunteer_roles_category_id_fkey FOREIGN KEY (category_id) REFERENCES public.volunteer_master_roles(id) ON DELETE CASCADE;
    `);
    await client.query(`
...
CREATE TABLE IF NOT EXISTS volunteers (
    ...
    password text NOT NULL,
    user_id integer REFERENCES public.clients(id)
);
...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    FOREIGN KEY (user_id) REFERENCES public.clients(id)
);
