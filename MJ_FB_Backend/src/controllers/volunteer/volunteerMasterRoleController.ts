...
export async function createVolunteerMasterRole(req: Request, res: Response, next: NextFunction) {
...
export async function updateVolunteerMasterRole(req: Request, res: Response, next: NextFunction) {
  const { id } = req.params;
  const { name } = req.body as { name?: string };
  if (!name) {
    return res.status(400).json({ message: 'name is required' });
  ...
  try {
    ...
    if ((result.rowCount ?? 0) === 0) {
      return res.status(404).json({ message: 'Master role not found' });
    }
...
export async function deleteVolunteerMasterRole(req: Request, res: Response, next: NextFunction) {
  const { id } = req.params;
  try {
    const result = await pool.query(
    ...
    res.json({ message: 'Master role deleted' });
  } catch (error) {
